...
angular.module('listenone').controller('NavigationController', [
  ...
  ($scope, $timeout, $rootScope) => {
    ...
    // Bind i18n strings used by the folder management UI onto $rootScope so they
    // are available in the (inherited) playlist view scope.
    const folderI18nKeys = [
      '_ADD_LOCAL_FOLDER',
      '_LOCAL_FOLDERS',
      '_MANAGE_LOCAL_FOLDERS',
      '_LOCAL_FOLDERS_DESC',
      '_DONE',
      '_REMOVE_FOLDER',
      '_RESCAN_FOLDER',
    ...
    folderI18nKeys.forEach((key) => {
      $rootScope[key] = i18next.t(key);
    });
    // The folder dialog renders `localFolders.length`; make sure it is always
    // an array before the first load.
    $rootScope.localFolders = [];

    const getLocalRemote = () => {
      const remote = require('@electron/remote');
      const remoteFunctions = remote.require('./functions.js');
      return { remote, remoteFunctions };
    };

    const buildLocalTrack = (fp, md) => ({
      id: `lmtrack_${fp}`,
    ...
    // Path helpers. Folder identity is compared on a normalized form so the
    // same folder spelled differently (E:\Music vs E:\music, trailing slash)
    // is always treated as one folder.
    const normPath = (s) => (s || '').replace(/\\/g, '/').toLowerCase();
    const trimSlash = (s) => (s || '').replace(/\/+$/, '');
    const folderKey = (s) => trimSlash(normPath(s));
    const sameFolder = (a, b) => folderKey(a) === folderKey(b);

    // True when a track's `sound_url` points at a file inside `folder`.
    // The trailing separator check keeps `E:\music2` out of `E:\music`.
    const isUnderFolder = (soundUrl, folder) => {
      if (!soundUrl) {
        return false;
      }
      const base = trimSlash(normPath(`file://${folder}`));
      return normPath(soundUrl).startsWith(`${base}/`);
    };

    // Scan a folder, read tags for every music file and merge into the
    // local playlist. When `persist` is true the folder is added to the
    // monitored-folders list (localStorage). Auto-scan happens on add;
    // afterwards only manual rescan touches it.
    $scope.scanAndAddFolder = (folder, persist) => {
      if (!isElectron()) {
        return;
      }
      const { remoteFunctions } = getLocalRemote();
      notyf.info(i18next.t('_FOLDER_SCANNING'));
      remoteFunctions
        ...
        .then((files) => {
          ...
          files.forEach((fp) => {
            remoteFunctions
              .readAudioTags(fp)
              .then((md) => {
                tracks.push(buildLocalTrack(fp, md));
              })
              .catch(() => {})
              .finally(() => {
                pending -= 1;
          ...
          function finishScan() {
            // A scan is authoritative for its own folder: drop every stored
            // track that lives under it (stale files, and any entry written
            // earlier from the same folder under a different path spelling),
            // then add this scan's set. The result is exactly one entry per
            // file in the folder, so re-scanning can never accumulate copies.
            const seen = {};
            ...
            const stored = localStorage.getObject('lmplaylist_reserve');
            if (stored && Array.isArray(stored.tracks)) {
              stored.tracks = stored.tracks.filter(
                (tr) => !isUnderFolder(tr.sound_url, folder)
              );
              localStorage.setObject('lmplaylist_reserve', stored);
            }
            MediaService.addPlaylist('lmplaylist_reserve', fresh).success(
              () => {
                if (persist) {
                  const folders =
                    localStorage.getObject('localmusic_folders') || [];
                  if (!folders.some((f) => sameFolder(f, folder))) {
                    folders.push(folder);
                    localStorage.setObject('localmusic_folders', folders);
                  }
                }
                $scope.loadLocalFolders();
                $scope.refreshLocalPlaylist();
                notyf.success(
                  i18next.t(persist ? '_FOLDER_ADDED' : '_FOLDER_RESCANNED')
                );
              }
            );
          }
    ...
    // Folder management lives on the "Local Music" sidebar row rather than in
    // the playlist body: adding a folder is a one-off command, managing the
    // library is a setting, so the song list stays purely about songs.
    $scope.openLocalFolderManager = (event) => {
      // That row opens the playlist on click; the entry icon must not also
    ...
    $scope.addLocalFolder = () => {
      if (!isElectron()) {
        return;
      }
      const { remote } = getLocalRemote();
      remote.dialog
        .showOpenDialog({
          title: i18next.t('_ADD_LOCAL_FOLDER'),
        ...
        .then((result) => {
          ...
          result.filePaths.forEach((folder) => {
            $scope.scanAndAddFolder(folder, true);
          });
        })
        .catch(() => {});
    };

    // Remove a monitored folder and every track that belongs to it.
    $scope.removeLocalFolder = (folder) => {
      ...
      const folders = (localStorage.getObject('localmusic_folders') || [])
        .filter((f) => !sameFolder(f, folder));
      localStorage.setObject('localmusic_folders', folders);
      $scope.loadLocalFolders();
      $scope.refreshLocalPlaylist();
      notyf.success(i18next.t('_FOLDER_REMOVED'));
    };

    $scope.rescanLocalFolder = (folder) => {
      $scope.scanAndAddFolder(folder, false);
    };

    $scope.rescanAllLocalFolders = () => {
      ($rootScope.localFolders || []).forEach((f) => {
        $scope.scanAndAddFolder(f.path, false);
      });
    };

    // Load the monitored folders (with per-folder track counts) into $rootScope
    // so both playlist-view templates can render them. Entries pointing at the
    // same folder are collapsed, so a folder is never listed - or counted -
    // twice.
    $scope.loadLocalFolders = () => {
      const stored = localStorage.getObject('localmusic_folders') || [];
      const seenFolders = {};
      const folders = stored.filter((f) => {
      ...
      const playlist = localStorage.getObject('lmplaylist_reserve');
      const tracks = playlist && Array.isArray(playlist.tracks)
        ? playlist.tracks
        : [];
      const counts = {};
      folders.forEach((f) => {
      ...
      tracks.forEach((tr) => {
        folders.forEach((f) => {
          if (isUnderFolder(tr.sound_url, f)) {
            counts[f] += 1;
          }
        });
      });
      $rootScope.localFolders = folders.map((f) => ({
        path: f,
        count: counts[f] || 0,
      }));
      $scope.$evalAsync();
    ...
    // Refresh the songs list if the user is currently viewing the local playlist.
    $scope.refreshLocalPlaylist = () => {
      if ($scope.list_id === 'lmplaylist_reserve') {
        MediaService.getPlaylist('lmplaylist_reserve').success((data) => {
